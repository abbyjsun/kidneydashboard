...
# Excludes recipient/leakage variables AND variables 100% missing in Non-TX donors
# ============================================================
...
cols_to_exclude <- c(
  ...
  # creatinine / post-transplant outcomes
  "Cr1M", "Cr3M", "Cr6M", "Cr12M",
  "Cr3years", "Cr5years", "Cr10years",
  "Cr_at_last_Followup",
  
  # admin / follow-up / outcome timing
  "Last_follow_up_date",
  "Death_date",
  "Graft_loss_date",
  
  # unavailable for Non-TX recommendation
  blocked_non_tx_vars
)
...
# Force donor clinical variables to remain eligible if present and numeric
donor_clinical_available <- donor_clinical_vars[
  donor_clinical_vars %in% names(df_transplanted)
]

donor_clinical_numeric <- df_transplanted %>%
  select(any_of(donor_clinical_available)) %>%
  select(where(is.numeric))

features_numeric <- bind_cols(
  donor_clinical_numeric,
  features_numeric %>% select(-any_of(names(donor_clinical_numeric)))
)

# Remove features with too much missingness in transplanted training data
feature_missing_rate <- colMeans(is.na(features_numeric))

features_numeric <- features_numeric[, feature_missing_rate <= 0.20, drop = FALSE]

cat("Features after <=20% missingness filter:",
    ncol(features_numeric), "\n")

cat("Numeric candidate features after leakage/non-TX exclusion:",
    ncol(features_numeric), "\n")
...
run_mrmr <- function(outcome_vector,
  ...
  selected <- selected[!stringr::str_detect(selected, bad_regex)]
  selected <- selected[!(selected %in% cols_to_exclude)]
